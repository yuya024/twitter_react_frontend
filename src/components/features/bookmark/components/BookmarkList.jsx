export const BookmarkList = (props) => {
  const { bookmarks, tweetDateFormat, submitBookmark } = props;

  return bookmarks.map((bookmark) => (
    <div key={bookmark.id} className="grid grid-cols-8 border-b py-2">
        ...
        <div className="grid grid-cols-4">
          <div className="col-span-1 flex">
            ...
            {bookmark.tweet.comment_count !== 0 && (
              <p className="px-1 text-sm">{bookmark.tweet.comment_count}</p>
            )}
          </div>
          <div className="col-span-1 flex">
            <svg
            ...
            {bookmark.tweet.retweet_count !== 0 && (
              <p className="px-1 text-sm">{bookmark.tweet.retweet_count}</p>
            )}
          </div>
          <div className="col-span-1 flex">
            <svg
            ...
            {bookmark.tweet.favorite_count !== 0 && (
              <p className="px-1 text-sm">{bookmark.tweet.favorite_count}</p>
            )}
          </div>
          <div className="col-span-1">
            <button onClick={() => submitBookmark(bookmark.id)}>
              <svg
                viewBox="0 0 24 24"
                aria-hidden="true"
                className="h-5 w-5"
